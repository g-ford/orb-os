...
bool decode(const uint8_t *png, uint32_t len, bool alpha, uint8_t *&out, int &w, int &h, const char *tag) {
    if (!ensure_decoder()) { Serial.printf("[menu_sprite] %s: decoder alloc failed\n", tag); return false; }
    s_alpha = alpha;
    if (s_png->openRAM((uint8_t *)png, len, line_cb) != PNG_SUCCESS) { Serial.printf("[menu_sprite] %s: open failed\n", tag); return false; }
    ...
    if (!out) { Serial.printf("[menu_sprite] %s: buffer alloc failed\n", tag); s_png->close(); return false; }
    s_buf = out; s_w = w;
    const int r = s_png->decode(nullptr, 0);
    s_png->close();
    if (r != PNG_SUCCESS) { Serial.printf("[menu_sprite] %s: decode failed\n", tag); return false; }
    Serial.printf("[menu_sprite] %s: decoded %dx%d (%u KB)\n", tag, w, h, (unsigned)(bytes / 1024));
    return true;
}
